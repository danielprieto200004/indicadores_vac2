...
export default async function ReportePage({
  ...
  const areaBreakdown = areaList
    .map((a) => {
      ...
      const indicators = [
        ...aContribs.map((c) => {
          const l = latestById.get(c.id) ?? null;
          ...
          return {
            ...
            meta: c.meta_area,
            current_value: l?.current_value ?? null,
            percent: l?.percent ?? null,
            traffic_light: (l?.traffic_light ?? null) as TrafficLight | null,
            date: l?.period_end ?? null,
          };
        }),
        ...aOwn.map((c) => {
          const l = ownLatestById.get(c.id) ?? null;
          return {
            id: c.id,
            type: "own" as const,
            indicador: c.indicador_area,
  ...
  return (
    <div>
      ...
      <div className="print:hidden mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-card p-4 shadow-sm">
        <div>
          <h1 className="text-xl font-semibold">Reporte de Indicadores · {selectedYear}</h1>
          <p className="text-sm text-muted-foreground">Generado el {generatedAt}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <PrintButton year={selectedYear} />
        </div>
      </div>

      {/* ── Documento de reporte ── */}
      <div className="bg-white text-gray-900 rounded-xl border shadow-sm print:shadow-none print:border-0 print:rounded-none overflow-hidden">
        ...
        <div className="px-8 py-10 space-y-12 md:px-10">
          ...
          <section>
            ...
            {areaBreakdown.length === 0 ? (
              <p className="text-sm text-gray-500 py-4">
                Sin datos de área para {selectedYear}.
              </p>
            ) : (
              <div className="overflow-hidden rounded-xl border">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {areaBreakdown.map((a, i) => {
                      const critical = a.indicators.filter(ind => ind.traffic_light === "rojo").length;
                      const atRisk = a.indicators.filter(ind => ind.traffic_light === "naranja").length;
                      const tl: TrafficLight | null =
                        a.indicators.length === 0 ? null
                        : critical > 0 ? "rojo"
                        : atRisk > 0 ? "naranja"
                        : a.completed === a.total ? "verde"
                        : "naranja";
                      return (
                        <tr key={a.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/40"}>
                          ...
                          <td className="px-3 py-3 text-center tabular-nums">
                            <span className="text-amber-600 font-semibold">{atRisk}</span>
                            {critical > 0 && (
                              <span className="ml-1 text-red-600 font-semibold">({critical} críticos)</span>
                            )}
                          </td>
                          ...
                          <td className="px-3 py-3 text-center"><TlBadge tl={tl} /></td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          <div className="border-t pt-6 text-center text-xs text-gray-400 space-y-1">
            <p className="font-medium text-gray-500">
              Indicadores VAC · Año {selectedYear}
            </p>
