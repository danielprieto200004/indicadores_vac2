...
import { createOwnUpdate } from "@/app/app/propios/[id]/actions";
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2">
      <div className="md:col-span-2 rounded-xl border border-input bg-background p-4 text-sm">
        <div className="text-xs text-muted-foreground">Meta del indicador</div>
        <div className="mt-1 text-base font-semibold">
          {metaValue ?? "—"}{" "}
          {metaDesc ? <span className="text-sm font-normal text-muted-foreground">— {metaDesc}</span> : null}
        </div>
        <div className="mt-2 text-xs text-muted-foreground">
          Registra tu avance con fecha, semáforo, observación y evidencia.
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Label htmlFor="current_value">¿Cuánto llevas de la meta? (valor)</Label>
        <Input
          id="current_value"
          name="current_value"
          type="number"
      ...
      <div className="space-y-2 md:col-span-2">
        ...
        <Input id="comment" name="comment" required placeholder="Describe el avance del periodo, soporte o novedades." />
      </div>
